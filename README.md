...
4. 一般メンバーから休止中ゲームの復帰希望を受け付ける場合は、次を実行します。
   ...
   - `月間予定を編集`: Discord Activityで今月・翌月の日曜始まりカレンダーを開きます。サーバー全員の予定を閲覧でき、本人の予定だけを直接変更できます。スマートフォンでは日付詳細が下部シートになります。
   - `月間予定（週表示）`: Activityの準備中・障害時にも使える従来の週単位編集です。
   - `候補日程を見る`: 稼働中ゲームを選び、そのゲームを「遊びたい」に登録している現メンバーの `○ / △ / ×` の人数を、月間予定のタイムゾーン（通常はJST）における当日以降の日付順（同日は昼、夜の順）で最大10件表示します。候補を1件選ぶと、一般メンバーの誰でも対応するゲームチャンネルへ募集を送信できます。
   ...
   Activityの月間予定は同じサーバーの現在の非Botメンバー全員に共有されます。日付詳細では未入力と未登録を区別し、自分以外の予定は編集できません。今月内の過去日も編集でき、連続範囲を確認して基本予定へ戻せます。変更は自動保存され、他の人の変更は約5秒間隔で取得します。

   ゲームの希望編集・復帰希望の操作画面は本人にだけ表示され、公開パネルには希望者の名前を表示しません。休止中ゲームの復帰希望が設定人数に達しても自動では再稼働せず、管理者の確認操作が必要です。
